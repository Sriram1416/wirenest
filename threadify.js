...
// IMPORTANT: Replace the 'https://your-production-backend-url.com' with the actual deployed backend URL
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (isLoggedIn && savedEmail) {
        // Immediately set currentUser with the stored ID so cart checks work
        currentUser = {
            id: savedId,
            email: savedEmail,
            user_metadata: {
...
function renderProducts() {
    ...
    let displayProducts = products.filter(p => p.blockType === 'normal');
                    ...
                    ${displayProducts.map(product => {
                        ...
                        <div class="product-card" onclick="showProductModal('${product.id}')">
                            ...
                            <div class="product-info">
                                <h3>${product.name}</h3>
                                <div class="product-price">${currency.symbol}${product.price}</div>
...
// Open Customization for Specific Block
function openCustomizationForBlock(blockType, productId) {
    ...
    document.getElementById('modalProductPrice').textContent = `${currency.symbol}${priceConfig[blockType].small} `;

    // Reset quantity
    document.getElementById('modalQuantity').value = 1;

    // Reset customization selections
    document.querySelector('input[name="designType"][value="normal"]').checked = true;
    document.querySelector('input[name="size"][value="small"]').checked = true;

    // Reset color checkboxes - ensure at least one is selected
    const colorCheckboxes = document.querySelectorAll('input[name="color"]');
    colorCheckboxes.forEach(checkbox => {
        checkbox.checked = checkbox.value === 'green';
    });

    // Update price based on default small size for the block
    updateModalPrice(blockType, 'small');

    // Show customization section, hide normal actions
...
// Update modal price based on size selection for specific block
function updateModalPrice(blockType, size) {
    const price = priceConfig[blockType][size];
    document.getElementById('modalProductPrice').textContent = `${currency.symbol}${price} `;
}
...
function toggleCustomColor(color) {
    ...
    if (colorIndex > -1) {
        ...
        if (customizationSelections.colors.length < 3) {
...
async function addCustomizedToCart() {
    ...
    const colorRadio = document.querySelector('input[name="customColor"]:checked');
    ...
    const selectedColor = colorRadio.value;
        ...
        try {
            await fetch(`${BACKEND_URL}/cart/add`, {
                ...
                body: JSON.stringify({
                    ...
                    quantity: quantity,
                    customization_choices: { size: selectedSize, colors: [selectedColor], price: finalPrice }
                })
            });
            await loadCartFromStorage(); // Sync the UUIDs from Postgres immediately
        } catch (e) { console.error("DB Cart Sync Error:", e); }
...
function calculateCustomPrice() {
    ...
    if (sizeRadio && colorRadio) {
        ...
        priceDiv.textContent = 'Price: Missing Selection';
...
function showProductModal(productId) {
    console.log('🔍 MODAL OPEN DEBUG - Product ID:', productId);
    ...
    console.log('✅ Set currentProduct to:', JSON.stringify(currentProduct, null, 2));

    // Update modal content with product's actual details
    document.getElementById('modalProductName').textContent = product.name;

    if (product.blockType === 'customized') {
        const opts = product.customization_options || {};
        const sizes = opts.sizes || {};
        let colorArr = opts.colors || [];

        // Defensive parsing in case the database stored it as a string
        if (typeof colorArr === 'string') {
            try {
                colorArr = JSON.parse(colorArr);
            } catch (e) {
                colorArr = colorArr.split(',').map(s => s.trim());
            }
        }
        if (!Array.isArray(colorArr)) colorArr = ["red", "blue", "green", "yellow", "black", "white"];

        document.getElementById('dynamicCustomPrice').textContent = 'Price: Select Size';

        // Inject sizing tiers
        document.getElementById('customSizeOptions').innerHTML = Object.entries(sizes).map(([key, val], idx) => `
            <label class="size-option">
                <input type="radio" name="customSize" value="${key}" data-price="${val}" onchange="calculateCustomPrice()">
                <span>${key.charAt(0).toUpperCase() + key.slice(1)}</span>
            </label>
        `).join('');

        // Inject color options
        document.getElementById('customColorOptions').innerHTML = colorArr.map((col, idx) => `
            <label class="color-option" style="margin-right: 15px; display: inline-flex; align-items: center; gap: 5px;">
                <input type="radio" name="customColor" value="${col}" onchange="calculateCustomPrice()">
                <span class="color-swatch" style="background: ${col}; width: 20px; height: 20px; border-radius: 50%; border: 1px solid #ccc; display: inline-block;"></span>
        ...
        // Globally update the hardcoded HTML Description Element
        const descEl = document.querySelector('.product-details-description');
        if (descEl) descEl.textContent = product.long_description || product.description || product.short_description || '';

        // Hide static price for custom block types
        ...
        try {
            const opts = product.customization_options || {};
            const sizes = opts.sizes || {};
            let colors = opts.colors || ["red", "blue", "green", "yellow", "black", "white"]; // Renamed to 'colors' to avoid conflict with 'colorArr' above
            ...
            document.getElementById('customSizeOptions').innerHTML = Object.entries(sizes).map(([key, val]) => `
                <label class="size-option" style="cursor: pointer; display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
                    <input type="radio" name="customSize" value="${key}" data-price="${val}" onchange="calculateCustomPrice()">
                    <span>${key.charAt(0).toUpperCase() + key.slice(1)}</span>
                </label>
            `).join('');

            // Inject colors dynamically
            let colorsArray = ["red", "blue", "green", "yellow", "black", "white"];
            if (colors && colors.length > 0) { // Use 'colors' here, which is from opts.colors
                if (Array.isArray(colors)) {
                    ...
                    try {
                        ...
                        colorsArray = colors.split(',').map(s => s.trim());
                    }
                }
            }
            ...
            document.getElementById('customColorOptions').innerHTML = colorsArray.map(col => `
                <label class="color-option" style="margin-right: 15px; display: inline-flex; align-items: center; gap: 5px;">
                    <input type="radio" name="customColor" value="${col}" onchange="calculateCustomPrice()">
                    <span class="color-swatch" style="background: ${col}; width: 20px; height: 20px; border-radius: 50%; border: 1px solid #ccc; display: inline-block;"></span>
                    <span style="text-transform: capitalize; font-size: 14px;">${col}</span>
                </label>
            `).join('');
        ...
        // Prioritize dynamic price from database updates, otherwise fallback to config
        let finalPrice = product.price;
        if (finalPrice === undefined && product.blockType && product.size && priceConfig[product.blockType]) {
            finalPrice = priceConfig[product.blockType][product.size];
        }

        const staticPriceEl = document.getElementById('modalProductPrice');
        if (staticPriceEl) {
            staticPriceEl.style.display = 'block';
            staticPriceEl.textContent = `${currency.symbol}${finalPrice}`;
        }

        const descEl = document.querySelector('.product-details-description');
        if (descEl) descEl.textContent = product.long_description || product.description || product.short_description || '';

        document.getElementById('modalDescription').textContent = product.long_description || product.description || product.short_description || '';

        // Display product's actual size safely
        const sizeDisplay = document.getElementById('modalSizeDisplay');
        if (sizeDisplay) {
            const safeSize = product.size || 'Standard';
            sizeDisplay.textContent = safeSize.charAt(0).toUpperCase() + safeSize.slice(1);
        }

        // Display product's actual price in alternate UI block if it exists
        const priceDisplay = document.getElementById('modalPriceDisplay');
        if (priceDisplay) {
            priceDisplay.textContent = `${currency.symbol}${finalPrice}`;
        }

        // Display product's actual colors
        const colorDisplay = document.getElementById('modalColorDisplay');
        if (colorDisplay && product.colors) {
            const colorsArray = Array.isArray(product.colors) ? product.colors : [];
            colorDisplay.textContent = colorsArray.map(color =>
                color.charAt(0).toUpperCase() + color.slice(1)
            ).join(' | ');
        }
    ...
    // Update modal image with gallery
    ...
    modalImageContainer.innerHTML = `
        <div class="product-gallery">
            ...
            <div class="gallery-controls">
                <button onclick="navigateGallery(-1)" class="gallery-btn prev-btn">‹</button>
                <button onclick="navigateGallery(1)" class="gallery-btn next-btn">›</button>
            </div>
        </div >
    `;
